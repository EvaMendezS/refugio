...
const Analytics = (() => {
  ...
  function drawLineChart(canvas, puntos, opciones = {}) {
    ...
    const pad = 28;
    const color = opciones.color || getComputedStyle(document.body).getPropertyValue('--salvia-fuerte').trim() || '#8FA98C';
  ...
  function drawBarChart(canvas, items, opciones = {}) {
    ...
    const { ctx, w, h } = _prepCanvas(canvas);
    const pad = 24;
    const color = opciones.color || getComputedStyle(document.body).getPropertyValue('--rosa-fuerte').trim() || '#E8A6B4';
